...
class VanillaDataManager(DataManager, Generic[TDataset]):
    ...
    def print_train_data_info(self):
        # print("self.train_dataset.cameras: ",vars(self.train_dataset.cameras).keys())
        # print("vars(self.train_dataset) \n",vars(self.train_dataset).keys())
        # print("self.train_dataset[0] ", self.train_dataset[0])
        # print self.train_dataset.shape
        # I want to know how many items in self.train_datset
        print("len(self.train_dataset) ", len(self.train_dataset))
        print("self.train_dataset[0].keys() ", self.train_dataset[0].keys())
        # I want to know what are include in the self.train_datset
        print("self.train_dataset[0].keys() ", self.train_dataset[0].keys())
        print("self.train_dataset[0]['image'].shape ", self.train_dataset[0]["image"].shape)
        print("self.train_dataset[25].image_idx: ")
        print(self.train_dataset[25]["image_idx"])
        # print("self.train_dataset[0]['image'] ", self.train_dataset[0]["image"])

        # print("self.train_dataset.cameras[0]: ", self.train_dataset.cameras[0])
        print("self.train_dataset.cameras[25].keys(): ")
        print(list(self.train_dataset.cameras[25].__dict__.keys()))

        # print(dir(self.train_dataset))
        print(dir(self.train_dataset))

        print("*" * 100)
        image_print_id = 25
        print("image_print_id: ", image_print_id)
        print("Image Filename:", self.train_dataset.image_filenames[image_print_id])
        print("Image Index:", self.train_dataset[image_print_id]["image_idx"])
        print(type(self.train_dataset.cameras[image_print_id].camera_to_worlds))
        print(self.train_dataset.cameras[image_print_id].camera_to_worlds.shape)
        print("Camera to Worlds Data:\n", self.train_dataset.cameras[image_print_id].camera_to_worlds)
        # print("self.train_dataset.cameras[0]['camera_to_world'].shape: ", self.train_dataset.cameras[0]["camera_to_world"].shape)
        # print("self.train_dataset.cameras[0]['camera_to_world']: ", self.train_dataset.cameras[0]["camera_to_world"])
    ...
    def setup_train(self):
        ...
        # print(vars(self.train_image_dataloader))
        self.train_pixel_sampler = self._get_pixel_sampler(self.train_dataset, self.config.train_num_rays_per_batch)
        self.train_camera_optimizer = self.config.camera_optimizer.setup(
            num_cameras=self.train_dataset.cameras.size, device=self.device
        )
        print("self.config.camera_optimizer.mode: ", self.config.camera_optimizer.mode)

        # print("self.train_image_dataloader.num_images_to_sample_from: ", self.train_image_dataloader.num_images_to_sample_from)
        # print("self.train_dataset.cameras.size ",self.train_dataset.cameras.size)
        # print("self.train_dataset.image_filenames ",self.train_dataset.image_filenames)
        # print("self.train_dataset.image_filenames.size ",len(self.train_dataset.image_filenames))
        attributes = dir(self.train_dataset.cameras)
        print("self.train_dataset.cameras attributes: ", attributes)
        # print("self.train_dataset.cameras.camera_to_worlds: ", self.train_dataset.cameras.camera_to_worlds)
        print("self.train_dataset.camers.distortion_params: ", self.train_dataset.cameras.distortion_params)
    ...
    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        # print("Size of 'image_idx': ", image_batch['image_idx'].shape)
        # print("image_batch['image_idx']:\n", image_batch['image_idx'])
        # print("Size of 'image': ", image_batch['image'].shape)
        # 获取第一个图像的左上角的一个 10x10 区域
        # top_left_corner = image_batch['image'][0, :5, :5]

        # 打印此区域的形状和内容
        # print("Shape of the top-left corner:", top_left_corner.shape)
        # print("Content of the top-left corner:", top_left_corner)

        assert self.train_pixel_sampler is not None
        assert isinstance(image_batch, dict)
        batch = self.train_pixel_sampler.sample(image_batch)
        # print("train_pixel_sampler batch:\n",batch)
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        if self.config.camera_optimizer.mode != "off":
            ...
            # TODO: PRINT PARAMS
            # for name, param in self.train_camera_optimizer.named_parameters():
            #     print("*** name, params.size *** ",name, param.size())
        else:
            assert len(camera_opt_params) == 0
