...
class CameraOptimizerConfig(InstantiateConfig):
    ...
    mlp_hidden_units: Optional[int] = 64
    mlp_num_layers: Optional[int] = 2
...
class CameraOptimizer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.config.mode == "off":
            ...
            print("We Are in the SO3xR3 SETTING MODE\n")
            print("self.num_cameras: ", self.num_cameras)
            print("self.pose_adjustment.shape: ", self.pose_adjustment.shape)
        ...
        #     self.mlp = nn.Sequential(*self.mlp_layers)
            ...
            self.embed = sinusoidal_encoding(self.num_cameras, self.config.embedding_dim).to(self.device).float()
            # print("self.embed.dtype:  ",self.embed.dtype)
            ...
            for _ in range(self.config.mlp_num_layers - 1):
                ...
                self.mlp_layers_r.append(nn.ReLU())
            # self.mlp_layers_r.append(nn.Dropout(p=0.2))
            self.mlp_layers_r.append(nn.Linear(self.config.mlp_hidden_units, 3))
            self.mlp_r = nn.Sequential(*self.mlp_layers_r).float()
            ...
            for _ in range(self.config.mlp_num_layers - 1):
                ...
                self.mlp_layers_t.append(nn.ReLU())
            # self.mlp_layers_t.append(nn.Dropout(p=0.2))
            self.mlp_layers_t.append(nn.Linear(self.config.mlp_hidden_units, 3))
            self.mlp_t = nn.Sequential(*self.mlp_layers_t).float()
        ...
        if config.position_noise_std != 0.0 or config.orientation_noise_std != 0.0:
            ...
            # print("noise add to self.num_cameras: ", self.num_cameras)
    ...
    def forward(
        self,
        indices: Int[Tensor, "cameras_indices"],
    ) -> Float[Tensor, "cameras_indices 3 4"]:
        ...
        # print("indices of cameras to optimize: ", indices)
        # print("indices of cameras to optimize shape: ", indices.shape)

        # Apply learned transformation delta.
        if self.config.mode == "off":
            pass
        elif self.config.mode == "SO3xR3":
            # camera_indices = torch.arange(self.num_cameras, device=self.pose_adjustment.device)
            # outputs.append(exp_map_SO3xR3(self.pose_adjustment[camera_indices, :]))
            # print("Pose adjustment shape: ", self.pose_adjustment.shape)
            # print("pose_adjustment: ", self.pose_adjustment)
            # print("Indices shape: ", indices.shape)
            # print("Indices : ",indices)
            # print("Pose adjustment shape: ", self.pose_adjustment[indices, :].shape)
            result = exp_map_SO3xR3(self.pose_adjustment[indices, :])
            # print("Result shape: ", result.shape)
            # print("\nResult1000: ", result[1000])
            # print("\nResult1001: ", result[1001])
            # print("\nResult1002: ", result[1002])
            outputs.append(exp_map_SO3xR3(self.pose_adjustment[indices, :]))
            # print("Total number of elements in outputs:", len(outputs))
        ...
        #     outputs.append(exp_map_SO3xR3(tangent_vector))   
            ...
            # Indices might need to be reshaped, normalized or otherwise preprocessed
            camera_indices = torch.arange(self.num_cameras, device=self.device)
            # print("camera_indices.dtype ",camera_indices.dtype)
            x = self.embed[camera_indices]  # (num_cams, embedding_dim)
            # print("indices.shape : ",camera_indices.shape)
            ...
            for idx in range(len(camera_indices)):
                r_i = r[idx]  # get the rotation of i-th frame
                t_i = t[idx]  # get the translation of i-th frame
                # c2w_i = make_c2w(r_i, t_i)  # Output: (4, 4) -> camera pose for i-th frame
                rt_i = torch.cat((r_i, t_i))
                c2w_outputs.append(rt_i)  
            # Converting the list of outputs to a tensor
            # print outputs size()
            c2w_outputs = torch.stack(c2w_outputs) 
            # c2w_outputs = c2w_outputs.type(torch.float32)
            # print("c2w_outputs.dtype: ", c2w_outputs.dtype)
            # print("c2w_outputs.shape ",c2w_outputs.shape)
            # print("c2w_outputs: ",c2w_outputs)
            result = exp_map_SO3xR3(c2w_outputs[indices, :])
            # print("mlp result shape: ", result.shape)
            # outputs.append(exp_map_SO3xR3(c2w_outputs[indices, :]))
        ...
        # print("indices.shape[0]: ", indices.shape[0])
        if len(outputs) == 0:
